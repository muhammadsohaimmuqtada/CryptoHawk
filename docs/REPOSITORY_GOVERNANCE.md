...
The repository currently treats exact-head CI as a merge invariant even when GitHub-side protection is unavailable. GitHub-side protection should still be enabled because policy must not depend on operator memory.
...
4. require the complete CryptoHawk CI matrix on the resulting PR.

Do not replace a commit pin with a floating branch or unverified third-party action.

## Dependency updates

Dependabot is configured for Python, npm and GitHub Actions. Dependency PRs must pass the same security, reliability and release-qualification matrix as application changes.

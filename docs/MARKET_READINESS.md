...
- [x] Exportable executive and engineering reports
...
Reporting is workspace-scoped and Viewer-readable, and is generated on demand from authoritative active observation state rather than stale historical findings. Executive JSON/CSV and print-ready self-contained HTML summarize asset coverage, severity, quantum exposure, active policy, remediation accountability, overdue/unowned work and 30-day cryptographic drift. Engineering JSON and formula-injection-safe CSV expose evidence-level cryptographic observations, risk/policy results, first/last seen state and remediation linkage. A current-state CycloneDX 1.7 CBOM is exported from the same active evidence set, so resolved exposures disappear consistently across engineering reports and CBOM output. Source snippets, evidence metadata and connector secrets are deliberately excluded from the reporting bundle. Tenant isolation, Viewer RBAC, HTML escaping, CSV formula neutralization and current-state semantics are covered by CI.
...
All repository P0 implementation gates above are now complete. Until the serious-impact gate is demonstrated in a representative real-world pilot, README/release language must describe CryptoHawk as a commercial-pilot candidate rather than generally available production software.
